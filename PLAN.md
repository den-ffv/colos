...
- [x] **Автозаповнення клієнта** — при виборі клієнта підтягувати його типову адресу відправлення.  
  Реалізовано: при виборі клієнта з dropdown адреса клієнта авто-заповнює поле pickup якщо воно порожнє.

- [x] **Збереження чернетки** — форма автоматично зберігає прогрес у `localStorage` (не втрачається при перезавантаженні).  
  Реалізовано: debounced auto-save кожні 1.5 сек, кнопка "Очистити чернетку" в topbar.

- [ ] **Шаблони договорів** — зберегти типовий договір як шаблон і створювати нові на його основі.

- [x] **Валідація в реальному часі** — поля валідуються при виході з них (on-blur), а не тільки при сабміті.  
  Реалізовано: `blur()` хелпер на всіх обов'язкових полях, першим скролить до помилки при сабміті.

- [ ] **Підказки-tooltip** — для складних полів (маржа, собівартість, тип виконання) додати інформаційні підказки.

- [x] **Автоматичний розрахунок зарплати водія** — при виборі водія автоматично розраховувати `estimatedSalaryCost` на основі `pay_rate` / `pay_type`.  
  Реалізовано (26.04.2026): `GET /lookups` повертає `payRate`/`payType`; Mapbox duration зберігається у `durationHours`; `useEffect` розраховує по формулі залежно від типу (PER_KM, PER_HOUR, PER_DAY, FIXED); badge "авто" + hint під полем; ручна зміна скидає hint; в режимі редагування не перезаписує збережене значення.
...
| Тиждень 7 — UX / Форма | 🔄 В процесі (авто-зарплата ✅, чернетка ✅, валідація ✅, автозаповнення клієнта ✅) |
